...
class TestInversionSolution(object):
    ...
    def test_check_types(self, crustal_solution_fixture):
        ...
        model = sol.model
        assert model.rupture_rates["Annual Rate"].dtype == pd.Float32Dtype()
    ...
    def test_get_rupture_ids_intersecting_crustal(self, crustal_solution_fixture):
        ...
        ruptures = model.get_rupture_ids_intersecting(polygon)
        all_rupture_ids = list(model.ruptures.index)
    ...
    def test_slip_rate_soln(self, crustal_solution_fixture):
        ...
        assert (model.average_slips.index == model.average_slips["Rupture Index"]).all()
        assert len(model.fault_sections_with_solution_slip_rates) == len(model.fault_sections)
        assert model.fault_sections_with_solution_slip_rates.loc[0, "Solution Slip Rate"] == approx(
    ...
    def test_target_slip_rates(self, crustal_solution_fixture):
        ...
        assert "SlipRateStdDev" not in model.fault_sections.columns
        assert "Section Index" not in model.fault_sections.columns
...
class TestSmallPuyInversionSolution(object):
    def test_new_puysegur_filter_solution(self, puysegur_small_fixture):
        ...
        new_sol = InversionSolution.filter_solution(sol, sol.model.ruptures.head()["Rupture Index"])
        assert isinstance(new_sol, InversionSolution)
        print(new_sol.model.rupture_rates)
        print(sol.model.rupture_rates)

        # What is this test doing??
        assert sol.model.rupture_rates.head().all().all() == new_sol.model.rupture_rates.all().all()

    def test_check_indexes(self, puysegur_small_fixture):
        model = puysegur_small_fixture.model
        assert model.rupture_rates.index == model.rupture_rates["Rupture Index"]
        assert model.ruptures.index == model.ruptures["Rupture Index"]
        assert model.indices.index.all() == model.indices["Rupture Index"].all()
        # assert sol.rupture_rates["Rupture Index"].dtype == pd.UInt32Dtype()
        # assert sol.ruptures["Rupture Index"].dtype == pd.UInt32Dtype()
        # assert sol.indices["Rupture Index"].dtype == pd.UInt32Dtype()
    ...
    def test_shapes(self, puysegur_small_fixture):
        model = puysegur_small_fixture.model
        print(model.rupture_rates)
        assert model.rupture_rates.shape == (10, 2)
        print(model.indices)
        assert model.indices.shape == (10, 273)
        print(model.ruptures)
        assert model.ruptures.shape == (10, 5)
