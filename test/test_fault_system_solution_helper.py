...
def test_parent_fault_vs_section_participation(crustal_solution_fixture):
    ...
    subsections = helper.subsections_for_parent_fault_names([fault_name])
...
def test_explore_zero_rates(crustal_solution_fixture):
    ...
    assert df0[df0==0].shape[0] == 0 # there are no 0 values here, so bu implication every section has at least one rupture
