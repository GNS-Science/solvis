...
def fetch_toshi_files(work_folder, file_ids ):
    ...
    api = ToshiFile(API_URL, S3_URL, None, with_schema_validation=False, headers=headers)
    file_map={}
    for fid in file_ids:
        click.echo(f'fetching {fid}')
        file_detail = api.get_file(fid, with_file_url=True)
        click.echo(file_detail)
        fname = pathlib.Path(work_folder, file_detail['file_name'])
        file_map[fid] = fname
        if fname.exists():
            click.echo(f'skippinng {fname}')
            continue
    ...
    for branch in branch.branches:
        solutions.append(
            BranchInversionSolution.new_branch_solution(
                InversionSolution().from_archive(filemap[branch.inversion_solution_id]),
                branch = branch
            )
        )
...
@click.group()
@click.option('--fault_system', '-fs', default='PUY', type=click.Choice(['PUY', 'HIK', 'CRU']))
