...
class CompositeSolutionFile(InversionSolutionFile):
    ...
    DATAFRAMES = [InversionSolutionFile.RATES_PATH, InversionSolutionFile.RUPTS_PATH, InversionSolutionFile.INDICES_PATH, COMPOSITE_RATES_PATH]

    def _write_dataframes(self, zip_archive:zipfile.ZipFile, reindex:bool =False):
        if reindex:
            data_to_zip_direct(zip_archive, reindex_dataframe(self._composite_rates).to_csv(), self.COMPOSITE_RATES_PATH)
